...
# calculate mean absolute error of model's predictions
mae_dt3 <- mae(test$price, predict_dt3)
mae_dt3

# calculate root mean squared error of model's predictions
rmse_dt3 <- rmse(test$price, predict_dt3)
rmse_dt3
...
# calculate adjusted r squared
RsqAdj_dt3 <- 1 - (1-Rsq_dt3)*((nrow(test)-1)/(nrow(test)-1-1))
RsqAdj_dt3
```

#### Tuning CP to 0.000001
...
# calculate mean absolute error of model's predictions
mae_dt4 <- mae(test$price, predict_dt4)
mae_dt4

# calculate root mean squared error of model's predictions
rmse_dt4 <- rmse(test$price, predict_dt4)
rmse_dt4
...
# calculate adjusted r squared
RsqAdj_dt4 <- 1 - (1-Rsq_dt4)*((nrow(test)-1)/(nrow(test)-1-1))
RsqAdj_dt4
```
...
#### Tuning
The following parameters can be tuned to increase the performance of a GBM model: <br />
1. n.trees: number of trees <br />
2. bag.fraction: proportion of observations to be sampled in each tree <br />
3. n.minobsinnode: minimum number of observations in the trees terminal nodes <br />
4. interaction.depth: maximum nodes per tree <br />
5. shrinkage: learning rate <br />
<br />
...
```
Based on the cross validation error, the optimal number of trees when predicting should be `r pred_cv`. However, this RMSE is less than the RMSE of the original model. Therfore, this ntree should be left as is.
...
```
The best model is model is `r model_compare$ModelName[[1]]`, a `r model_compare$ModelType[[1]]` model using the formula price vs `r as.character(formula(model_compare$ModelName[1])[3])`. It has an RMSE of `r model_compare$RMSE[1]`, a MAE of `r model_compare$MAE[1]`, and an adjusted R square of `r model_compare$AdjRSquare[1]`.
