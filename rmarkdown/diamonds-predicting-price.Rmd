...
head(data)
```
Looks like there is a mix of numeric and categorical data. The numeric data (carat, depth, table, x, y, & z) are doubles (dbl), which means they are numerics with decimal places. Price is a numeric integer, so it will not have any decimal places. The categorical data (cut, color, and clarity) are ordered (ord), which means it has a specific order, such as best to worst. Below is a quick statistical summary of the data:
```{r}
summary(data)
```
A minimum, 1st quartile, median, mean, 3rd quartile, and maximum is provided for all the numeric data. For example, the median price of diamonds in this data set is `r median(data$price)`. A data count is provided for all categorical data. For example, there are `r data %>% filter(cut=="Ideal") %>% count() %>% pull()` diamonds that are Ideal cut in this data set. Since all the categorical data is ordered, the counts are in its pre-specified order. Cut is from `r min(levels(data$cut))`
