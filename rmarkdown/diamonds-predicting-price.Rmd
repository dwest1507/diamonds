...
## 3D Scatter Plots
This next section will explore the data using 3-dimensional scatter plots. <br /><br />
...
```
Based on the cross validation error, the optimal number of trees when predicting should be `r ntree_opt_cv`. However, this RMSE is less than the RMSE of the original model. Therfore, this ntree should be left as is.
...
```
The best model is `r model_compare$ModelName[[1]]`, a `r model_compare$ModelType[[1]]` model using the formula price vs `r as.character(formula(model_compare$ModelName[1])[3])`. It has an RMSE of `r model_compare$RMSE[1]`, a MAE of `r model_compare$MAE[1]`, and an adjusted R square of `r model_compare$AdjRSquare[1]`.
