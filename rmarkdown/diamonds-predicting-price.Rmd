...
# Model Exploration
Time to create some predictive machine learning models!
```{r, echo=F}
...
model_lm1 <- lm(price ~ carat,        # The formula is price as a function of carat
                data)                 # The data is an object called data

summary(model_lm1) # Stats summary of the model
```
The created formula is Price = `r model_lm1$coefficients[[1]]` + `r model_lm1$coefficients[[1]]` * Carat. The adjusted R-squared of `r summary(model_lm1)[[9]]`isn't too bad, and carat's p-value passes the T test.
